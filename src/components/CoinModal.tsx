...
export default function CoinModal({ coin, onClose }: CoinModalProps) {
    ...
    fetch(`/api/coins/issues?id=${coin.id}`)
      ...
      .then(data => {
        if (Array.isArray(data)) {
          // Filter out issues without years and sort by year
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-stone-900/80 backdrop-blur-sm" onClick={onClose}>
      <div 
      ...
      >
            ...
            <div className="flex justify-center items-center gap-4 sm:gap-8 relative z-10">
              {obverseSrc && (
                <div className="w-32 h-32 sm:w-48 sm:h-48 rounded-full overflow-hidden border-4 border-amber-500/30 shadow-2xl bg-stone-800">
                  <Image src={obverseSrc} alt="Аверс" width={200} height={200} className="object-cover w-full h-full" unoptimized />
                </div>
              )}
              {reverseSrc && reverseSrc !== obverseSrc && (
                <div className="w-32 h-32 sm:w-48 sm:h-48 rounded-full overflow-hidden border-4 border-amber-500/30 shadow-2xl bg-stone-800">
                  <Image src={reverseSrc} alt="Реверс" width={200} height={200} className="object-cover w-full h-full" unoptimized />
                </div>
              )}
            </div>
          )}
        ...
        {/* Details Grid */}
        <div className="p-6 sm:p-8 space-y-8 bg-[url('/parchment.jpg')] bg-cover">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div className="bg-white/80 backdrop-blur p-4 rounded-2xl border border-amber-200 shadow-sm text-center">
              ...
              <p className="font-bold text-stone-800 text-sm mt-1">
                {coin.min_year === coin.max_year ? coin.min_year : `${coin.min_year} - ${coin.max_year}`}
              </p>
            </div>
            ...
            <div className="bg-white/80 backdrop-blur p-4 rounded-2xl border border-amber-200 shadow-sm text-center">
              ...
              <p className="font-bold text-stone-800 text-sm mt-1">{coin.weight ? `${coin.weight} г` : '—'}</p>
            </div>
